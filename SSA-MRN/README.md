...
우선은 HSI 초해상도 확장보다 **SSA-MRN 원 논문의 pansharpening 결과 재현**을 진행합니다. 공식 저장소의 네트워크 구현을 확인하고, 논문에 공개된 학습 조건·데이터 처리·평가 절차를 재구성합니다. 재현 과정과 결과를 기록한 뒤 RGB–HSI 확장은 다음 단계에서 검토합니다.
...
원 논문 재현 결과와 누락 설정을 정리한 다음에 RGB–HSI 초해상도 확장을 별도 실험으로 판단합니다. 해당 확장은 PAN/MS 원 문제와 입력·출력, 데이터셋, 평가 방법이 달라 원 논문 재현 설정에 섞지 않습니다.
